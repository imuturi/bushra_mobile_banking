...
import 'landing-login-3-login.dart';

class LoginSecurityQuestions extends StatefulWidget {
  const LoginSecurityQuestions({super.key});

  @override
  State<LoginSecurityQuestions> createState() => _LoginSecurityQuestionsState();
}

class _LoginSecurityQuestionsState extends State<LoginSecurityQuestions> {
  ...
  // Example question set (ID → Question)
  final Map<String, String> _questions = {
    "Whats your pet name": "What's your pet name?",
    "Whats your favourite car": "What's your favourite car?",
    "Whats the name of your best friend": "What's the name of your best friend?",
  };
  ...
  void _onContinue(String phone) {
    ...
    debugPrint("Sending to API: $payload");
    verifySecurityQuestion(phone, _selectedQuestionId!, _answerController.text.trim());
  }
  ...
  Future<void> verifySecurityQuestion(String phone, String questionAsked, String questionAnswered) async {
    try {
      setState(() {
        isLoading = true;
      });
      ...
      final response = await apiLogin.customerSecurityQuestionVerify(
        SecurityQuestionValidationRequest(
          ...
          transactionDetails: TransactionDetails(
            ...
            phoneNumber: phone,
            securityQuestions: [
              {'questionAsked': questionAsked, 'questionAnswered': questionAnswered }, //TODO remove the numbering E.g questionAnswered2 = questionAnswered
      ...
      if (response != null) {
        ...
        } else if(RemoteConfigService.isProdBuild == 'true' && response['response_code'] != '00'){
          setState(() {
            isLoading = false;
            Navigator.pop(context);
            showSnackBar(context, 'Error: ${response['data']['response']['response']}', Colors.red);
          });
        }else if(RemoteConfigService.isProdBuild == 'false' && response['response_code'] != '00'){
          setState(() {
            isLoading = false;
            Navigator.pop(context);
            showAlertDialogSecurityQuestion(context); // TODO SHOW SUCCESS MESSAGE
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              decoration: InputDecoration(
                ...
                helperStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.w100, color: Colors.black54),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(5.0)),
              ),
              value: _selectedQuestionId,
              items: _questions.entries.map((entry) {
                return DropdownMenuItem<String>(
                  value: entry.key,
                  child: Text(entry.value, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w100)),
                );
              }).toList(),
              onChanged: (value) {
                setState(() {
